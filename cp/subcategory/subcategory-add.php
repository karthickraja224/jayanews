<?php
// subcategory/subcategory-add.php
...
$page_title  = __('add_subcat_title');
$breadcrumbs = [__('subcat_title') => BASE_URL . 'subcategory/subcategory-list.php', __('add_subcat_title') => null];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$cat_id)     { flashError(__('err_cat_required')); }
        ...
        $slug = uniqueSlug('tbl_subcategories', $slug);
        db()->insert(
            "INSERT INTO tbl_subcategories (category_id, name_english, name_tamil, slug, sort_order, status) VALUES (?,?,?,?,?,?)",
            'isssii', $cat_id, $name_en, $name_ta, $slug, $sort, $status
        );
        logActivity('create', 'subcategory', "Sub-category added: $name_en");
        flashSuccess(__('msg_subcat_added'));
...
<div class="page-header">
    <div class="page-header-left"><h1><i class="fas fa-plus-circle"></i> <?= __('add_subcat_title') ?></h1></div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-control" required>
                            ...
                            <option value="<?= $c['id'] ?>" <?= (isset($_POST['category_id']) && $_POST['category_id']==$c['id']) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($c['name_english']) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" name="name_english" class="form-control" value="<?= htmlspecialchars($_POST['name_english'] ?? '') ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label"><?= __('label_name_ta') ?></label>
                        <input type="text" name="name_tamil" class="form-control" value="<?= htmlspecialchars($_POST['name_tamil'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Slug</label>
                        <input type="text" name="slug" class="form-control" value="<?= htmlspecialchars($_POST['slug'] ?? '') ?>" placeholder="sub-category-slug">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px">
                        ...
                        <div class="form-group">
                            ...
                            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:10px">
                                <input type="checkbox" name="status" <?= !isset($_POST['name_english']) ? 'checked' : (isset($_POST['status']) ? 'checked' : '') ?>> <?= __('label_active') ?>
                            </label>
                        </div>
                    </div>
                </div>
